...
def lee_mask(img : np.ndarray, filter_height=4, filter_width=40) -> np.ndarray:
    """
    Implement the Lee mask method for finger vein recognition.

    :param image: Input grayscale image of the finger.
    ...
    :return: Binary mask of the finger region.
    """
...
def identify_significant_height_differences(mask: np.ndarray, height_difference_threshold: float) -> list:
    """
...
def get_avg_height(mask: np.ndarray):
    img_h, img_w = mask.shape
    half_img_h = img_h // 2  # Middle point of the image (integer division)
    ...
    return avg_top_upper_half

def adjust_columns_to_average_height(mask: np.ndarray, target_avg_height: float, batch_size=20) -> np.ndarray:
    """
...
def adjust_columns_to_average_height_2(mask: np.ndarray, target_avg_height: float, batch_size=20, tolerance_percentage=0.15) -> np.ndarray:
    """
